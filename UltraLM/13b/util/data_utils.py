...
class PromptIterableDataset(IterableDataset):
    ...
    def tokenize_example(self, example):
        ...
        # if reasoning data, do not add prefixes like "User" or "Assistant"
        ...
        for i, c in enumerate(example["data"]):
            ...
            if i % 2 == 1:
                # model
                c_input = self.start_token + self.prefix(tags[i])
                ...
                if i == 0:
                    # no start token
                    c_new = self.tokenizer.bos_token + self.prefix(tags[i]) + c + end_token
                else:
                    c_new = self.start_token + self.prefix(tags[i]) + c + end_token
